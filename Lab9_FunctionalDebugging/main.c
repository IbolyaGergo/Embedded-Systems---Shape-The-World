...
unsigned long PrevStateSW1, PrevStateSW2;   // save the states of switches
...
int main(void){  unsigned long i;
  ...
  while(1){
		SW1 = GPIO_PORTF_DATA_R&0x10; // read PF4
		SW2 = GPIO_PORTF_DATA_R&0x01; // read PF0
		...
		if (PrevStateSW1 != SW1 || PrevStateSW2 != SW2) {
		  PrevStateSW1 = SW1;
		  PrevStateSW2 = SW2;
			if (i < 50) {
				Data[i] = GPIO_PORTF_DATA_R&0x13;    // record PF4, 1, 0
