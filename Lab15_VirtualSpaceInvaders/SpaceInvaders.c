...
// distance on the LCD x direction.
// ADC value measured on PE1.
// Input: sample  12-bit ADC sample
// Output: 32-bit distance
unsigned long ADCtoPlayerX(unsigned long sample){
	// simulation
  return (sample + MAX_X - 1 - PLAYERW)>>6; // pos x in (1-65)
	// real board
...
void Init(void){ int i;
  ...
  for(i=0;i<4;i++){
	...
	// player x pos from ADC
	ADCdata = ADC0_In();
	PLAYERX = ADCtoPlayerX(ADCdata);
}
...
// main1 is a simple main program to debug the ADC interface
int main1(void){ 
  ...
  while(1){
		Nokia5110_ClearBuffer();
//		Nokia5110_PrintBMP((83 - PLAYERW)*(ADCdata + 1), 47, PlayerShip0, 0);
		ADCdata = ADC0_In();
		PLAYERX = ADCtoPlayerX(ADCdata);
		Nokia5110_PrintBMP(PLAYERX, 47, PlayerShip0, 0);
